...
def immunize(
    ...
    """
    target_pv = -portfolio.present_value(y)
    target_duration = portfolio.duration(y)

    pvs = np.array([a.present_value(y) for a in available_assets])
    durations = np.array([a.duration(y) for a in available_assets])
